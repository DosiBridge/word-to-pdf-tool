...
import { redactPdf, downloadFile } from '@/lib/api';
...
export default function RedactPdfPage() {
    ...
    const handleConvert = async () => {
        ...
        try {
            const blob = await redactPdf(file, searchText, replacement);
            downloadFile(blob, `redacted_${file.name}`);
            setSuccess(true);
        } catch (err) {
            ...
            <main className="flex-grow container mx-auto px-4 py-12">
                <div className="max-w-4xl mx-auto text-center mb-12">
                    ...
                    <p className="text-lg text-zinc-600 dark:text-zinc-400">
                        Permanently black out sensitive text in your PDF documents.
                    </p>
                </div>
                ...
                <div className="w-full max-w-2xl mx-auto p-6 bg-white dark:bg-zinc-900 rounded-lg shadow-lg border border-zinc-200 dark:border-zinc-800">
                    <div className="space-y-4">
                        ...
                        <button
                            ...
                            disabled={!file || !searchText || loading}
                            className="w-full px-6 py-3 bg-red-600 hover:bg-red-700 disabled:bg-zinc-400 text-white font-semibold rounded-md transition-colors disabled:cursor-not-allowed"
                        >
                        ...
                        {success && (
                            <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md">
                                <p className="text-sm text-green-800 dark:text-green-300">
                                    Text redacted successfully! File downloaded.
                                </p>
                            </div>
                        )}
                    </div>
